...
- Article pages must also be created, edited, reordered, moved, and deleted through simple YAML/content files.
- Avoid hardcoded endpoint lists, article lists, request examples, response examples, languages, statuses, or navigation sections.
...
- Own navigation tree generated from YAML/content files.
- Article pages generated from YAML/content files.
...
- API Reference and Articles must have independent navigation trees.
...
- Informational Articles navigation is generated from its own content/config files.
...
- Article pages support rich formatted content.
- Active navigation item is highlighted.
- Navigation sections are expanded by default on first entry.
- Expanded/collapsed state is preserved during navigation.
- Adding a new endpoint or article requires only adding/editing content files, not changing application code.
