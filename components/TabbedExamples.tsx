...
import type { EndpointCodeExample, HttpMethod } from "@/lib/openapi";
...
export function TabbedExamples({ method, requestExamples, responseExamples }: TabbedExamplesProps) {
  ...
  const [copied, setCopied] = useState<"request" | "response" | undefined>();
  ...
  async function copyValue(kind: "request" | "response", value: unknown) {
    await navigator.clipboard.writeText(formatClipboardValue(value));
    setCopied(kind);
      ...
      <section className="example-card">
        <div className="example-card-header">
          ...
          <div className="example-card-actions">
            ...
            <button
              type="button"
              className={copied === "request" ? "example-icon-button example-icon-button-copied" : "example-icon-button"}
              aria-label="Copy request example"
              onClick={() => activeRequest ? copyValue("request", activeRequest.value) : undefined}
      ...
      <section className="example-card">
        <div className="example-card-header">
          ...
          <div className="example-card-actions">
            ...
            <button
              type="button"
              className={copied === "response" ? "example-icon-button example-icon-button-copied" : "example-icon-button"}
              aria-label="Copy response example"
              onClick={() => activeResponse ? copyValue("response", activeResponse.example) : undefined}
              disabled={!activeResponse || activeResponse.example === undefined}
            >
              <Copy size={14} />
            </button>
          </div>
